...
Canonical shared skills live in `agents/.agents/skills/`.

- When an agent supports local skills, prefer that directory as the source of truth.
- Agent-specific skills that do not belong in the shared set should stay in the agent's own config directory.
- If a tool requires a fixed local skills directory, point it at the shared skills with symlinks rather than duplicating files.
