---
name: codex-review
description: Use when the user says "/codex-review", "codex review my plan", "argue this plan with Codex", "adversarial plan review", "make Claude and Codex fight over the plan", or wants a second-model sanity check on an implementation PLAN before building something high-stakes (auth, schema, concurrency, migrations, payments). Requires an existing plan or clear idea — Claude loads it into PLAN.md, OpenAI Codex critiques it in a read-only sandbox (VERDICT:APPROVED or VERDICT:REVISE), and Claude revises in the same Codex session until approval or a MAX_ROUNDS cap, with human sign-off before code. For a requirements interview BEFORE the review, use /grill-me-codex instead. NOT for reviewing already-written CODE (that is /codex:review) and NOT for trivial changes.
---
